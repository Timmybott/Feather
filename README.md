...
- **Chats** — create any number of self-named chats (**admins** create them; everyone posts). Messages are **live** (updated in real time). A message's **sender name and avatar are clickable** and open that person's profile. Tag a teammate with **`@username`** — they get a **notification**, and the mention is a link to their profile; tag a file with **`#path`** and it opens **that exact file** in the Files tab. A bell in the header shows your unread mentions and assignments.
- **Tasks** — anyone creates a task, assigns one or more **members**, links an **open issue**, and comments on it; mark tasks done. Done tasks move into a collapsible **Completed** section. Assignees are **notified** when someone comments or changes the task. Comment authors, mentions and assignee avatars are all clickable.
- **To-do lists** — shared checklists, optionally linked to an issue and **assignable to members** (assignees are notified of changes). Delete a list when you're done; a fully-checked list moves into a collapsible **Completed** section.

**People are clickable everywhere** — chat senders/mentions, task-comment and issue-comment authors, and assignee avatars all jump straight to the profile.

When an **issue is closed**, the tasks and to-do lists linked to it are **auto-archived** (and restored if you reopen it), so finished work moves out of the way. Closing can also happen **automatically on deploy**: when committing you can tick one or more open issues, which pins them to that commit — once the commit ships from **Current Deploy**, those issues close themselves. It's the same on the desktop and the web.
...
- A **live homepage** — a Feather landing page that reads from the public GitHub API, so it's always current without a rebuild: an **OS-aware download** button (links straight to the Windows/macOS/Linux installer of the latest release), a **"What's new"** changelog (recent releases, Markdown-rendered), and **stats & contributors** (stars, forks, version, a contributor avatar wall) — plus the search box and feature highlights.
...
- **An account menu** — signed in, your avatar (top-right) opens a menu with **Your profile**, a **Your teams** list, and **Log out**.
...
- **"Open in desktop app"** — a project page can hand off to the installable app (via a `feather://` deep link) for the things the web can't do.
- **Web Deployments** — a web-capable project (Website/Node.js/Python/…) can be **put online**: from the desktop **Settings → Web Deployments**, enabling it publishes the project's latest deploy to Feather's own nginx server, reachable at **`https://feather.spcfy.eu/webdeployment/<slug>/`**. A green **● Live ↗** badge on the project page opens it. Re-publish after a deploy to update the live site.
- **Readable URLs** — clean, hash-free, shareable links: `feather.spcfy.eu/team/<slug>`, `/user/<username>`, `/project/<slug>` (old `#/t/…` links redirect automatically). Requires a one-line nginx SPA fallback — see [docs/RELEASING.md](docs/RELEASING.md).
...
| `0025_planning_assignees.sql` | To-do list assignees + triggers that notify task/to-do assignees on comments and changes |
| `0026_issue_autoclose.sql` | `close_deployed_issues()` — close issues pinned to a commit once that commit deploys |
