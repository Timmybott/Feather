...
**Web Deployments (v3.3, M59).**
- Ein web-fähiges Projekt kann online gestellt werden: `supabase/0021` ergänzt `projects.web_deploy`/`web_slug` + mitglieder-only `set_web_deploy()` (generiert einen stabilen, eindeutigen Slug). `feather-storage` bekommt zwei Aktionen: `publish-web` (holt den neuesten *released* Deploy-Snapshot aus `data/<team>/<project>/rollbacks/<bundle>.zip`, lädt ihn hoch und entpackt ihn nach `/webroot/webdeployment/<slug>/` auf dem Nginx-Server) und `unpublish-web` (löscht den Ordner) — beide mit `team_members`-Mitgliedscheck. Erreichbar unter `https://feather.spcfy.eu/webdeployment/<slug>/`. Desktop-`ProjectDetail` → Settings: „Web Deployments" (Enable&publish / Re-publish / Take offline, nur bei web-fähigem Server-Typ prominent), grünes **● Live ↗** Badge auf Projekt-Header (Desktop + Web). `cloud.setWebDeploy/publishWebDeployment/unpublishWebDeployment` + `webDeployUrl`. Der Server-Typ-Chip erscheint jetzt auch auf der Desktop-Projektseite.

**Lesbare, hashfreie URLs (v3.3, M57).**
- Der Web-Router wechselt von Hash- auf History-Routing (`web/lib/router.svelte.ts`): `/team/<slug>`, `/user/<username>`, `/project/<slug>`, `/search`, `/login`, `/settings`. Alte `#/t|u|p/…`-Links werden beim Laden auf die neuen Pfade umgeschrieben (`replaceState`). `supabase/0022` ergänzt eindeutige, generierte `slug`-Spalten auf `teams`/`projects` (Backfill + Insert-Trigger); `cloud.resolveTeamId/resolveProjectId/resolveUserId` lösen Slug/Username→id auf (UUID synchron, sonst Lookup), `web/App.svelte` rendert die (id-basierten) Profil-Komponenten nach Auflösung. Ausgehende Links (Suche, Header-Teamliste, Team-Chip) nutzen Slugs. Erfordert nginx-SPA-Fallback (`try_files $uri $uri/ /index.html;`, in `docs/RELEASING.md`); reale Pfade wie `/webdeployment/<slug>/` werden vorher bedient.

## 12. Planning-Tab, Server-Wizard & Webapp-Parität (v3.4)

**Planning/Organisations-Tab (M63).** Neuer projektinterner Tab (nur Team-Mitglieder). `supabase/0023` legt `planning_chats/messages/tasks/task_assignees/task_comments/todo_lists/todo_items` + eine `notifications`-Tabelle an, alles members-only RLS (Chat-Insert = Admin über `is_team_admin`), einen Auto-Archiv-Trigger (Issue geschlossen → verknüpfte Tasks/To-do-Listen `archived`, bei Reopen zurück) und Realtime auf `planning_messages`/`notifications`. `src/lib/planning.ts` kapselt die Helfer (Profile werden UI-seitig aus der Mitgliederliste aufgelöst). `PlanningTab.svelte` (geteilt) hat drei Sektionen: **Chats** (Admin erstellt; Live-Nachrichten via Realtime; `@username` → Notification, `#pfad` → Files-Tab), **Tasks** (Zuweisung von Mitgliedern + offenem Issue, Kommentare, done/archived), **To-do-Listen** (mit Issue verknüpfbar). `NotificationBell.svelte` (in beiden Headern) zeigt ungelesene Mentions/Zuweisungen live. Eingebunden in Desktop-`ProjectDetail` und Web-`WebProject`.

**Server-Wizard ohne Admin-Key (M64).** Da die Client-API keine Server erstellt, führt `NewServerWizard.svelte` (Desktop-Projects-Screen) durch die Spezifikation (Runtime/RAM/Disk/CPU/Standort), zeigt eine kopierbare Zusammenfassung + Panel-Link und leitet in den bestehenden Import-Flow über.

**Webapp-Parität (M65).** Die Web-Projektseite gleicht jetzt der Desktop-Version: 4 Statistik-Kacheln (offene Issues, Deploys, letzter Deploy, aktueller Commit / „not deployed"), Recent-Activity-Timeline, Metadaten-Sidebar und ein mitglieder-only **Settings-Tab** (Name/Beschreibung/Logo bearbeiten, Web Deployments verwalten). **Auto-Re-Publish** (`supabase/0024` `web_auto_publish`, Standard aus) als Schalter in Desktop- und Web-Settings; `DeployPanel` bekommt `onDeployed`, `ProjectDetail` re-published danach automatisch.

**Bugfixes (M66).** Desktop-Settings-Dialog (`DesktopSettings.svelte`, über das Account-Menü: Version, Update-Check, Benachrichtigungen, Account löschen) — vorher nur Web. Markdown-/Issue-Links öffnen im Systembrowser: `tauri-plugin-opener` + Klick-Interceptor im Desktop-Root (`src/App.svelte`).

**Planning-Politur, Zuweisungen & Auto-Close (v3.4.1, M67–M72).** *Anklickbare Personen überall:* im Chat öffnen Absender-Name, Avatar und jede `@`-Erwähnung das Profil — ebenso Task-Kommentar- und Issue-Kommentar-Autoren (`IssueThread` erhält `onOpenProfile`, weitergereicht über `IssuesPanel`) sowie Assignee-Avatare. *Profilbilder im Chat:* Nachrichten/Kommentare/Assignees zeigen `avatar_url` (Fallback: Initiale). *`#datei`-Erwähnungen* öffnen die **konkrete Datei**: `FileBrowser` bekommt `openPath` (parst Verzeichnis + Dateiname, navigiert, öffnet nach dem Laden); `onOpenFile(path)` in `PlanningTab` reicht den Pfad über `ProjectDetail`/`WebProject` durch. *Completed-Sektionen:* Tasks (Status `done`) und voll abgehakte To-do-Listen wandern in einen ausklappbaren Bereich (Svelte-Snippets teilen die Karten). *Zuweisbare To-do-Listen:* `supabase/0025` legt `planning_todo_list_assignees` an; `assignTodoList` benachrichtigt. *Assignee-Benachrichtigungen (M71):* DB-Trigger in `0025` benachrichtigen Task-Assignees bei Kommentar/Status-/Titeländerung und To-do-Assignees bei Item-Änderungen (Actor via `auth.uid()` ausgenommen). *Commit → Issue → Auto-Close (M72):* `CloudCommits` bietet offene Issues zum Ankreuzen; nach `finalizeCommit` pinnt `assign_issue_commit` sie an den Commit. Beim Release des Bündels aus **Current Deploy** schließt `close_deployed_issues(p_bundle)` (`supabase/0026`) alle offenen Issues, deren Commit im Bündel liegt — was per Bestandstrigger auch die verknüpften Tasks/To-do-Listen archiviert. *Archive-Button bei To-dos entfernt* (nur noch Löschen; Auto-Archiv via Issue-Close bleibt als eigener ausklappbarer Bereich).

**Neue Meilensteine (v3.4):** M63 (Planning-Tab + Notifications) · M64 (Server-Wizard) · M65 (Webapp-Parität + Auto-Re-Publish) · M66 (Desktop-Settings + Link-Opener) · M67 (anklickbare Personen) · M68 (Chat-Avatare) · M69 (Datei-Erwähnungen → Datei) · M70 (To-do-Rework: Archive weg, Assignees, Completed-Sektionen) · M71 (Assignee-Benachrichtigungen) · M72 (Commit → Issue Auto-Close) — abgeschlossen.

**Neue Meilensteine (v3.0):** M50 (`feather-panel` Edge Function) · M51 (`supabase/0019` öffentliches anon-Lesen) · M52 (Web-SPA: Homepage, Suche, Ansichts-Seiten, Konsole) · M53 (CI-Webroot-Deploy, Version 3.0.0 + Docs) · M54 (Web-Deploy-Fix · 3.1.0) · M55 (Web-CORS-Fix, Mitglieder-Gating, lebendige Homepage, Account-Menü · 3.2.0) · M56/M57/M58/M59/M60/M61 (Web-Fetch-Fix, lesbare URLs, Server-Typ, Web Deployments, Deep-Link, Team-Löschung, Settings · 3.3.0) — abgeschlossen.
